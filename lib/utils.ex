defmodule Beamulator.Utils do
  def increasing_int() do
    :erlang.unique_integer([:monotonic, :positive])
  end

  def pid_from_string(pid_string) do
    pid_string = if String.starts_with?(pid_string, "#PID") do
  ...
  defmodule Actors do
    ...
    def select_by_serial_id(serial_id) when is_integer(serial_id) do
      ...
      if Enum.count(found) > 1 do
        raise "Multiple actors with the same serial_id (#{serial_id}) found: #{inspect(found)}"
      end
